...
<body>
    ...
    <?php
    $user_type_reg = $_GET['user_type'];
    $submit = @$_POST['submit'];
    $username = strip_tags(@$_POST['username']);
    $email = strip_tags(@$_POST['email']);
    $password = strip_tags(@$_POST['password']);
    if ($user_type_reg == 'teacher') {
        $user_type = 'student';
    } else {
        $user_type = 'teacher';
    }
    ...
    $id_ref = strip_tags(@$_POST['id_ref']);
    $date = date("Y-m-d");
    $code = rand();
    if ($submit) {
                        ...
                        if (preg_match("/[A-Z]/", $password)) {
                            if (preg_match("/[a-z]/", $password)) {
                                if (preg_match("/\W/", $password)) {
                                    ...
                                    if ($user_type_reg == 'teacher') {
                                        $user_type_data_check = "SELECT * from teacher_data WHERE code='" . $_GET["code"] . "'";
                                        $user_type_data_result = mysqli_query($conn, $user_type_data_check);
                                        while ($row = mysqli_fetch_array($user_type_data_result)) {
                                            echo $user_type_reg_id = $row['teacher_id'];
                                           ...
                                           VALUES (NULL, '$username','$email','$hashedPwd', '$user_type', '$date')");
                                            if ($user_type_reg == "teacher") {
                                                mysqli_query($conn, "INSERT INTO `student_data`(`student_id`, `student_name`, `student_email`,`teacher_id_ref`, `created_at`)
                                           VALUES (NULL, '$username','$email','$user_type_reg_id', '$date')");
                                                SendMail($email, 'Hi!Welcome to GlowEdu... <br> <br> This is your student registration link!<br> <br> http://localhost/glowedu/new_registration.php?user_type=teacher&code=' . $code);
                                            } else {
                                                mysqli_query($conn, "INSERT INTO `teacher_data`(`teacher_id`, `teacher_name`, `teacher_email`, `code`, `university_ref_id`, `created_at`)
                                           VALUES (NULL, '$username','$email','$code', '$user_type_reg_id', '$date')");
                                            }
                                        }
                                    } else {
                                        $user_type_data_check = "SELECT university_id from university_data WHERE code='" . $_GET["code"] . "'";
                                        $user_type_data_result = mysqli_query($conn, $user_type_data_check);
                                        while ($row = mysqli_fetch_array($user_type_data_result)) {
                                            echo $user_type_reg_id = $row['university_id'];
                                            mysqli_query($conn, "INSERT INTO `user_data`(`id`, `username`, `email`, `password`, `user_type`, `created_at`)
                                           VALUES (NULL, '$username','$email','$hashedPwd', '$user_type', '$date')");
                                            if ($user_type_reg == "teacher") {
                                                mysqli_query($conn, "INSERT INTO `student_data`(`student_id`, `student_name`, `student_email`,`teacher_id_ref`, `created_at`)
                        ...
                        <div class="card-body p-md-5">
                            <div class="row justify-content-center">
                                <div class="col-md-10 col-lg-6 col-xl-5 order-2 order-lg-1">
                                    ...
                                    <form class="mx-1 mx-md-4" method="POST" action="<?php echo $_SERVER['PHP_SELF'] . "?user_type=" . $_GET['user_type'] . "&&code=" . $_GET['code'] ?>">
                                        ...
                                        <div class="d-flex flex-row align-items-center mb-4">
                                            ...
                                            <div class="form-outline flex-fill mb-0">
                                                <input type="text" id="form3Example3c" value="<?php echo $_GET["code"]; ?>" name="id_ref" disabled class="form-control" />
                                                <label class="form-label" for="form3Example3c">Code</label>
                                            </div>
                                        </div>
